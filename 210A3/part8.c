    ...
    while (mainCode[*PC - *origin].code != -4059) { 
        ...
        *IR = mainCode[*PC - *origin - 1].code;
        switch (mainCode[*PC - *origin - 1].opCode) {
            ...
            case (-28672):    // NOT 
                registers[mainCode[*PC - *origin - 1].DR] = ~(registers[mainCode[*PC - *origin - 1].SR1]);
                setcc(registers, mainCode, PC, origin, CC);
                //printStatus(registers, PC, IR, CC, origin);
                break;  
            ...
            case (0):    // BR 
                if ((mainCode[*PC - *origin - 1].n == *CC) || (mainCode[*PC - *origin - 1].z == *CC) || (mainCode[*PC - *origin - 1].p == *CC)) {
                    *PC = *PC + mainCode[*PC - *origin - 1].PCoffset9;
                } 
                printStatus(registers, PC, IR, CC, origin);
                break;  
        } 
